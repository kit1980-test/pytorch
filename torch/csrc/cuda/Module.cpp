...
PyObject* THCPModule_cuda_tunableop_enable(PyObject* _unused, PyObject* arg) {
  ...
  at::cuda::tunable::getTuningContext()->EnableTunableOp(THPUtils_unpackBool(arg));
  Py_RETURN_NONE;
  END_HANDLE_TH_ERRORS
}
...
PyObject* THCPModule_cuda_tunableop_numerics_check_enable(
  ...
  TORCH_CHECK(
      ...
      THPUtils_typename(arg));
  at::cuda::tunable::getTuningContext()->EnableNumericsCheck(THPUtils_unpackBool(arg));
  Py_RETURN_NONE;
  END_HANDLE_TH_ERRORS
}
...
PyObject* THCPModule_cuda_tunableop_write_file_on_exit(PyObject* _unused, PyObject* arg) {
  HANDLE_TH_ERRORS
  TORCH_CHECK(
      THPUtils_checkBool(arg),
      "cuda_tunableop_write_file_on_exit expects a bool, but got ",
      THPUtils_typename(arg));
  at::cuda::tunable::getTuningContext()->WriteFileOnExit(THPUtils_unpackBool(arg));
  Py_RETURN_NONE;
  END_HANDLE_TH_ERRORS
}
...
PyObject* THCPModule_cuda_tunableop_set_filename(
  ...
  bool insert_device_ordinal = false;
  if (!PyArg_ParseTuple(
          args,
          "O|p",
          &str,
  ...
  at::cuda::tunable::getTuningContext()->SetFilename(filename, insert_device_ordinal);
  Py_RETURN_NONE;
  END_HANDLE_TH_ERRORS
}
